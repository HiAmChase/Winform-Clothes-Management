...
namespace QuanLyQuanAo.DAO
{
    public class ProductDAO
    {
        ...
        public List<ProductInfo> findProduct(string name)
        {
            ...
            string query = string.Format("SELECT P.IDProduct,P.Name,T.Name AS [Type],B.Name AS [Branch]," +
                "C.Color,P.Unit,S.Size, P.Amount,P.PriceOut,P.PriceIn " +
                "FROM Product P INNER JOIN Type T ON T.IDType = P.IDType " +
                ...
                "WHERE B.name like N'%{0}%'or P.name like N'%{1}%' or C.color like N'%{2}%' " +
                "or T.name like N'%{3}%'", name,name,name, name);
